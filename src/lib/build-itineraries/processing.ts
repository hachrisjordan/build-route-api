import { AvailabilityFlight, OptimizedItinerary } from './types';
import { getClassPercentages } from './class-percentages';

export function getTotalDuration(flights: (AvailabilityFlight | undefined)[]): number {
  let total = 0;
...
export function getSortValue(
  ...
  if (['y', 'w', 'j', 'f'].includes(sortBy)) {
    return getClassPercentages(flightObjs as AvailabilityFlight[], reliability, minReliabilityPercent)[
      sortBy as 'y' | 'w' | 'j' | 'f'
    ];
  }
  return 0;
  ...
  for (const routeKey of Object.keys(itineraries)) {
    ...
    for (const date of Object.keys(itineraries[routeKey] || {})) {
      for (const itinerary of itineraries[routeKey]![date] || []) {
        ...
        const classPercentages = getClassPercentages(flightObjs as AvailabilityFlight[], reliability, minReliabilityPercent);
...
export function optimizedFilterSortSearchPaginate(
  ...
  if (
    query.stops?.length ||
    query.includeAirlines?.length ||
    query.excludeAirlines?.length ||
    query.maxDuration !== undefined ||
    query.minYPercent !== undefined ||
    query.minWPercent !== undefined ||
    query.minJPercent !== undefined ||
    query.minFPercent !== undefined ||
    query.depTimeMin !== undefined ||
    query.depTimeMax !== undefined ||
    query.arrTimeMin !== undefined ||
    query.arrTimeMax !== undefined ||
    query.includeOrigin?.length ||
    query.includeDestination?.length ||
    query.includeConnection?.length ||
    query.excludeOrigin?.length ||
    query.excludeDestination?.length ||
    query.excludeConnection?.length
  ) {
