...
export async function POST(req: NextRequest) {
  try {
    ...
    const parseResult = lhFSchema.safeParse(body);
    ...
    for (const entry of mergedResults) {
      ...
      // For LH flights, ensure at least Y or J availability
      if (flightPrefix === 'LH') {
    ...
    const valkeyKey = `lh-f-response:${hash}`;
