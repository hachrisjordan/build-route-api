...
# Override output table (e.g. sandbox copy) without editing code. Used by gflights_explore --supabase.
_ENV_EXPLORE_OUTPUT_TABLE = "GOOGLE_FLIGHTS_EXPLORE_OUTPUT_TABLE"
_ENV_EXPLORE_PAIRING_STATUS_TABLE = "GOOGLE_FLIGHTS_EXPLORE_PAIRING_STATUS_TABLE"


def get_supabase_explore_output_table() -> str:
    """Resolve explore destination prices table: env GOOGLE_FLIGHTS_EXPLORE_OUTPUT_TABLE or default."""
    raw = (os.environ.get(_ENV_EXPLORE_OUTPUT_TABLE) or "").strip()
    return raw if raw else SUPABASE_EXPLORE_OUTPUT_TABLE


def get_supabase_explore_pairing_status_table() -> str:
    """Resolve pairing status table: env GOOGLE_FLIGHTS_EXPLORE_PAIRING_STATUS_TABLE or default."""
    raw = (os.environ.get(_ENV_EXPLORE_PAIRING_STATUS_TABLE) or "").strip()
    return raw if raw else SUPABASE_EXPLORE_PAIRING_STATUS_TABLE


# Staging tables for IATA / airline codes seen on Explore that are not yet in `airports` / `airlines`.
_ENV_GFLIGHTS_MISSING_AIRPORTS_TABLE = "GOOGLE_FLIGHTS_EXPLORE_MISSING_AIRPORTS_TABLE"
_ENV_GFLIGHTS_MISSING_AIRLINES_TABLE = "GOOGLE_FLIGHTS_EXPLORE_MISSING_AIRLINES_TABLE"
GFLIGHTS_DEFAULT_MISSING_AIRPORTS_TABLE = "gflights_explore_missing_airports"
GFLIGHTS_DEFAULT_MISSING_AIRLINES_TABLE = "gflights_explore_missing_airlines"


def get_supabase_gflights_missing_airports_table() -> str:
    """Resolve staging table for unknown destination IATAs (not in `airports`)."""
    raw = (os.environ.get(_ENV_GFLIGHTS_MISSING_AIRPORTS_TABLE) or "").strip()
    return raw if raw else GFLIGHTS_DEFAULT_MISSING_AIRPORTS_TABLE


def get_supabase_gflights_missing_airlines_table() -> str:
    """Resolve staging table for unknown airline codes (not in `airlines`)."""
    raw = (os.environ.get(_ENV_GFLIGHTS_MISSING_AIRLINES_TABLE) or "").strip()
    return raw if raw else GFLIGHTS_DEFAULT_MISSING_AIRLINES_TABLE
...
def upsert_explore_pairing_status(
    ...
    status: str,
    *,
    pairing_table: Optional[str] = None,
    ...
    client = create_client(url, key)
    tbl = (pairing_table or "").strip() or get_supabase_explore_pairing_status_table()
    # One row per capture identity: same run can record roundtrip + oneway for the same region.
    # DB must have a single UNIQUE (run_id, origin_iata, destination_region, trip_type) — see
    # sql/google_flights_explore_pairing_status_unique.sql if your table still has conflicting indexes.
    res = (
        client.table(tbl)
        .upsert(row, on_conflict="run_id,origin_iata,destination_region,trip_type")
        .execute()
    )
    status_code = 200
...
def _canonical_explore_region_for_airport_row(explore_region: Any) -> Optional[str]:
    """Map `explore_region` on a row to a canonical `airports.region` label, if it matches."""
    if explore_region is None:
        return None
    s = str(explore_region).strip()
    if not s:
        return None
    for canonical in AIRPORTS_SUPABASE_REGIONS_ORDERED:
...
def ensure_missing_airports_for_gflights_explore_rows(
    ...
    *,
    ...
    """
    After an Explore run, for each destination IATA not in canonical `airports`, upsert into the
    staging table (default ``gflights_explore_missing_airports``). Does not modify `airports`.
    """
    if not rows:
        return
    try:
    ...
    tbl = (staging_table or "").strip() or get_supabase_gflights_missing_airports_table()

    first_row_by_dest: Dict[str, Dict[str, Any]] = {}
    dest_iatas: Set[str] = set()
    for row in rows:
        d = (row.get("airport") or "").strip().upper()
        if not IATA_REGEX.match(d):
            continue
        dest_iatas.add(d)
        if d not in first_row_by_dest:
            first_row_by_dest[d] = row
    ...
    attempts = max(1, int(execute_max_attempts))
    delay = max(0.0, float(execute_retry_delay))
    codes_sorted = sorted(dest_iatas)
    in_main: Set[str] = set()
    chunk_size = 120
    for i in range(0, len(codes_sorted), chunk_size):
        chunk = codes_sorted[i : i + chunk_size]
        response = None
        for att in range(attempts):
            try:
                response = (
                ...
                if debug:
                    print(
                        f"[debug] airports main lookup retry {att + 1}/{attempts}: "
    ...
    not_in_main = sorted(dest_iatas - in_main)
    if not not_in_main:
        return

    now_iso = datetime.now(timezone.utc).isoformat()
    in_staging: Set[str] = set()
    for i in range(0, len(not_in_main), chunk_size):
        chunk = not_in_main[i : i + chunk_size]
        response = None
        for att in range(attempts):
            try:
                response = (
                ...
                if debug:
                    print(
                        f"[debug] missing_airports staging lookup retry {att + 1}/{attempts}: "
                        f"{type(exc).__name__}: {exc}",
                        file=sys.stderr,
                    )
                if att + 1 >= attempts:
    ...
    to_refresh: List[str] = []
    for iata in not_in_main:
        if iata in in_staging:
            to_refresh.append(iata)
        else:
            row = first_row_by_dest.get(iata) or {}
            city = (row.get("city") or "").strip() or "Unknown"
            country = (row.get("country") or "").strip() or "Unknown"
            er = _canonical_explore_region_for_airport_row(row.get("explore_region"))
            er_s = er if er else None
            rec: Dict[str, Any] = {
                "iata": iata,
                "display_name": f"{city} ({iata})",
                "city_name": city,
                "country": country,
            ...
            if er_s is not None:
                rec["explore_region"] = er_s
            to_insert.append(rec)

    insert_chunk = 50
    for j in range(0, len(to_insert), insert_chunk):
        batch = to_insert[j : j + insert_chunk]
        for att in range(attempts):
            try:
                client.table(tbl).insert(batch).execute()
                if debug:
                    print(
                        f"[debug] missing_airports insert: {len(batch)} row(s) "
                        f"iata={[r['iata'] for r in batch]}",
                        file=sys.stderr,
                    )
                break
            except Exception as exc:
                if debug:
                    print(
                        f"[debug] missing_airports insert retry {att + 1}/{attempts}: "
                        f"{type(exc).__name__}: {exc}",
                        file=sys.stderr,
                    )
                if att + 1 >= attempts:
    ...
    for iata in to_refresh:
        ...
        if er:
            payload["explore_region"] = er
        for att in range(attempts):
            try:
                client.table(tbl).update(payload).eq("iata", iata).execute()
                break
            except Exception as exc:
                if debug:
                    print(
                        f"[debug] missing_airports update retry {att + 1}/{attempts} iata={iata}: "
                        f"{type(exc).__name__}: {exc}",
                        file=sys.stderr,
                    )
                if att + 1 >= attempts:
...
def ensure_missing_airlines_for_gflights_explore_pairs(
    pairs: Dict[str, str],
    *,
    staging_table: str | None = None,
    execute_max_attempts: int = 1,
    ...
    """
    For each airline ``code -> display name`` not present in canonical ``airlines``, upsert into the
    staging table (default ``gflights_explore_missing_airlines``). Does not modify ``airlines``.
    """
    if not pairs:
        return
    try:
    ...
    tbl = (staging_table or "").strip() or get_supabase_gflights_missing_airlines_table()
    codes_sorted = sorted({c.strip().upper() for c in pairs if c and c.strip()})
    if not codes_sorted:
        return
    ...
    for i in range(0, len(codes_sorted), chunk_size):
        ...
        for att in range(attempts):
            try:
                ...
                if debug:
                    print(
                        f"[debug] airlines main lookup retry {att + 1}/{attempts}: "
                        f"{type(exc).__name__}: {exc}",
                        file=sys.stderr,
                    )
                if att + 1 >= attempts:
    ...
    not_in_main = [c for c in codes_sorted if c not in in_main]
    if not not_in_main:
        return
    ...
    for i in range(0, len(not_in_main), chunk_size):
        ...
        for att in range(attempts):
            try:
                ...
                if debug:
                    print(
                        f"[debug] missing_airlines staging lookup retry {att + 1}/{attempts}: "
                        f"{type(exc).__name__}: {exc}",
                        file=sys.stderr,
                    )
                if att + 1 >= attempts:
    ...
    to_insert: List[Dict[str, Any]] = []
    to_refresh: List[str] = []
    for code in not_in_main:
        name = (pairs.get(code) or "").strip() or code
        if code in in_staging:
    ...
    for j in range(0, len(to_insert), insert_chunk):
        ...
        for att in range(attempts):
            try:
                ...
                if debug:
                    print(
                        f"[debug] missing_airlines insert: {len(batch)} row(s) "
                        f"code={[r['code'] for r in batch]}",
                        file=sys.stderr,
                    )
                break
            except Exception as exc:
                if debug:
                    print(
                        f"[debug] missing_airlines insert retry {att + 1}/{attempts}: "
                        f"{type(exc).__name__}: {exc}",
                        file=sys.stderr,
                    )
                if att + 1 >= attempts:
    ...
    for code in to_refresh:
        name = (pairs.get(code) or "").strip() or code
        payload = {"airline_name": name, "last_seen_at": now_iso}
        for att in range(attempts):
            try:
                client.table(tbl).update(payload).eq("code", code).execute()
                break
            except Exception as exc:
                if debug:
                    print(
                        f"[debug] missing_airlines update retry {att + 1}/{attempts} code={code}: "
                        f"{type(exc).__name__}: {exc}",
                        file=sys.stderr,
                    )
                if att + 1 >= attempts:
...
def _send_google_explore_alert_summary_email(
    *,
    to_email: str,
    routes: List[Dict[str, Any]],
    debug: bool = False,
) -> bool:
    """Send one summary email containing multiple newly-armed routes."""
    if not routes:
        return False
    resend_key = (os.getenv("RESEND_API_KEY") or "").strip()
    if not resend_key:
        if debug:
            print("[debug] google alert summary email skipped: RESEND_API_KEY missing", file=sys.stderr)
        return False
    to_addr = (to_email or "").strip()
    if not to_addr:
        return False
    ...
    first = routes[0]
    first_from = (
        f"{first.get('origin_city')} ({first.get('origin_iata')})"
        if (first.get("origin_city") or "").strip()
        else str(first.get("origin_iata") or "")
    )
    first_to = (
        f"{first.get('destination_city')} ({first.get('destination_iata')})"
        if (first.get("destination_city") or "").strip()
        else str(first.get("destination_iata") or "")
    )
    first_price = first.get("price")
    first_price_text = f"{float(first_price):.0f} USD" if isinstance(first_price, (int, float)) else "new price"
    if len(routes) == 1:
        subject = f"Mistake Fare Alert: {first_from} -> {first_to} from {first_price_text}"
    else:
        subject = f"Mistake Fare Alert: {len(routes)} routes turned way too cheap"

    def _route_line(route: Dict[str, Any]) -> str:
        origin_iata = str(route.get("origin_iata") or "").strip().upper()
        destination_iata = str(route.get("destination_iata") or "").strip().upper()
        origin_city = str(route.get("origin_city") or "").strip()
        destination_city = str(route.get("destination_city") or "").strip()
        from_label = f"{origin_city} ({origin_iata})" if origin_city else origin_iata
        to_label = f"{destination_city} ({destination_iata})" if destination_city else destination_iata
        trip = "Round-trip" if str(route.get("roundtrip") or "").strip().lower() == "roundtrip" else "One-way"
        airlines = route.get("airline_names") or []
        airlines_text = ", ".join([str(x).strip() for x in airlines if str(x).strip()]) or "Not available"
        price = route.get("price")
        price_text = f"{float(price):.0f} USD" if isinstance(price, (int, float)) else "n/a"
        return f"- {from_label} -> {to_label} | {trip} | {airlines_text} | {price_text}"

    route_lines = [_route_line(r) for r in routes]
    text_body = (
        "We found an unusually cheap price that might be a mistake fare.\n\n"
        "Newly armed routes in this run:\n"
        + "\n".join(route_lines)
    ...
    for route in routes:
        ...
        rows_html += (
            "<tr>"
            f"<td style='padding:8px 6px;color:#111827;font-size:13px;'>{from_label}</td>"
            f"<td style='padding:8px 6px;color:#111827;font-size:13px;'>{to_label}</td>"
            f"<td style='padding:8px 6px;color:#111827;font-size:13px;'>{trip}</td>"
            f"<td style='padding:8px 6px;color:#111827;font-size:13px;'>{airlines_text}</td>"
            f"<td style='padding:8px 6px;color:#059669;font-size:13px;font-weight:700;'>{price_text}</td>"
            "</tr>"
        )
    ...
    html_body = (
        ...
        "Mistake Fare Alert</div>"
        "<h2 style='margin:14px 0 6px 0;color:#111827;'>We found an unusually cheap price that might be a mistake fare.</h2>"
        "<p style='margin:0 0 12px 0;color:#6b7280;'>Newly armed routes in this run:</p>"
        "<table style='width:100%;border-collapse:collapse;border:1px solid #e5e7eb;'>"
        "<thead><tr>"
        "<th style='text-align:left;padding:8px 6px;color:#6b7280;font-size:12px;border-bottom:1px solid #e5e7eb;'>From</th>"
        "<th style='text-align:left;padding:8px 6px;color:#6b7280;font-size:12px;border-bottom:1px solid #e5e7eb;'>To</th>"
        "<th style='text-align:left;padding:8px 6px;color:#6b7280;font-size:12px;border-bottom:1px solid #e5e7eb;'>Trip type</th>"
        "<th style='text-align:left;padding:8px 6px;color:#6b7280;font-size:12px;border-bottom:1px solid #e5e7eb;'>Operated by</th>"
        "<th style='text-align:left;padding:8px 6px;color:#6b7280;font-size:12px;border-bottom:1px solid #e5e7eb;'>Price</th>"
        "</tr></thead>"
        f"<tbody>{rows_html}</tbody>"
        "</table>"
        "<p style='margin-top:18px;color:#6b7280;font-size:13px;'>You are receiving this because your google_alerts filters matched these routes.</p>"
        "</div></div></body></html>"
    )
    try:
        import requests
        ...
        if debug:
            print(
                f"[debug] google alert summary email skipped: requests unavailable ({type(exc).__name__}: {exc})",
                file=sys.stderr,
            )
        return False
    try:
        ...
        if debug:
            print(
                f"[debug] google alert summary email failed: status={resp.status_code} body={resp.text}",
                file=sys.stderr,
            )
        return False
    except Exception as exc:
        if debug:
            print(
                f"[debug] google alert summary email exception: {type(exc).__name__}: {exc}",
                file=sys.stderr,
            )
        return False
...
def recompute_way_too_cheap_for_buckets(
    ...
    *,
    ...
    """
    tbl = (output_table or "").strip() or get_supabase_explore_output_table()
    if not buckets:
        return
    try:
        from supabase import create_client
    ...
    while True:
        page_resp = _execute_with_retries(
            lambda off=offset: client.table(tbl)
            .select(
    ...
    for i in range(0, len(updates), chunk_size):
        ...
        _execute_with_retries(
            lambda c=chunk: client.table(tbl).upsert(
                c, on_conflict="id"
            ).execute()
        )
...
def sync_airline_google_pairs(
    ...
    *,
    ...
    """
    Fill/update `airlines.google` from carrier code -> display name (non-multi).
    Same rules as sync_airlines_google_from_explore_body.
    """
    if not pairs:
        return
    try:
...
def upsert_explore_destination_records(
    ...
    *,
    ...
    """
    Pairing-scoped way_too_cheap, upsert into explore output table, Resend/Discord alert transitions.
    Used by upsert_explore_csv_rows_to_supabase and gflights_explore (pre-built records).
    """
    if not records:
        return
    try:
    ...
    # ---- Pairing-scoped pre-upsert way_too_cheap compute ----
    try:
        # Load airports index once for region/country/city matching.
        airport_rows = getattr(
            client.table("airports").select("iata,region,country_code,city_name").execute(),
            "data",
            None,
        ) or []
        region_by_iata: Dict[str, str] = {}
        ...
        for row in airport_rows:
            ...
            if not iata:
                continue
            region_by_iata[iata] = (row.get("region") or "").strip()
            country_by_iata[iata] = (row.get("country_code") or "").strip().upper()
            city_by_iata[iata] = (row.get("city_name") or "").strip()

        def _row_key(row: Dict[str, Any]) -> tuple[str, str, str, str]:
            return (
                (row.get("origin_iata") or "").strip().upper(),
                (row.get("destination_iata") or "").strip().upper(),
                (row.get("roundtrip") or "").strip().lower(),
                (row.get("j") or "j").strip(),
            )

        # Bulk upsert must not repeat the same ON CONFLICT key in one statement (Postgres 21000).
        # Explore emits many rows per route (different dates); the table key is route × j only.
        def _dedupe_records_for_upsert_key(recs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
            by_k: Dict[tuple[str, str, str, str], Dict[str, Any]] = {}
            for r in recs:
                k = _row_key(r)
                prev = by_k.get(k)
                if prev is None:
                    ...
                    continue
                try:
                    p_new = int(r.get("price"))
                except (TypeError, ValueError):
                    p_new = None
                try:
                    p_old = int(prev.get("price"))
                except (TypeError, ValueError):
                    p_old = None
                take_new = False
                if p_old is None and p_new is not None:
                    take_new = True
                elif p_new is not None and p_old is not None and p_new < p_old:
                    take_new = True
                elif p_new == p_old and p_new is not None:
                    c_new, c_old = _to_float(r.get("cpm")), _to_float(prev.get("cpm"))
        ...
        records = _dedupe_records_for_upsert_key(records)
        if not records:
            return

        # Determine current pairing buckets from incoming records.
        pairing_buckets: set[tuple[str, str]] = set()
        record_origins: set[str] = set()
        record_dests: set[str] = set()
        record_roundtrips: set[str] = set()
        record_j_vals: set[str] = set()
        for r in records:
            key = _row_key(r)
            o, d, rt, jv = key
            if o:
                record_origins.add(o)
            if d:
                record_dests.add(d)
            if rt:
                record_roundtrips.add(rt)
            if jv:
                record_j_vals.add(jv)
            orr = region_by_iata.get(o, "").strip()
            drr = region_by_iata.get(d, "").strip()
            if orr and drr:
                pairing_buckets.add((orr, drr))

        # Existing state for touched row keys (transition baseline old flag).
        old_flag_by_key: Dict[tuple[str, str, str, str], Optional[bool]] = {}
        if record_origins and record_dests and record_roundtrips and record_j_vals:
            existing_resp = (
                client.table(tbl)
                .select("origin_iata,destination_iata,roundtrip,j,way_too_cheap")
                .in_("origin_iata", sorted(record_origins))
                .in_("destination_iata", sorted(record_dests))
                .in_("roundtrip", sorted(record_roundtrips))
                .in_("j", sorted(record_j_vals))
                .execute()
            )
            for row in getattr(existing_resp, "data", None) or []:
                old_flag_by_key[_row_key(row)] = row.get("way_too_cheap")

        # Pairing baseline pool: only rows with cpm < 20 in current pairing buckets.
        candidate_rows_by_key: Dict[tuple[str, str, str, str], Dict[str, Any]] = {}
        page_size = 2000
        off = 0
        while True:
            base_resp = (
                client.table(tbl)
                .select("origin_iata,destination_iata,roundtrip,j,cpm,price,airlines")
                .lt("cpm", 20)
                .range(off, off + page_size - 1)
                .execute()
            )
            base_rows = getattr(base_resp, "data", None) or []
            if not base_rows:
                break
            for row in base_rows:
                o, d, rt, jv = _row_key(row)
                if not o or not d or not rt:
                    continue
                orr = region_by_iata.get(o, "").strip()
                drr = region_by_iata.get(d, "").strip()
                if not orr or not drr:
                    continue
                if (orr, drr) not in pairing_buckets:
                    continue
                cpm = _to_float(row.get("cpm"))
                if cpm is None or cpm >= 20:
                    continue
                candidate_rows_by_key[(o, d, rt, jv)] = {
                    "origin_iata": o,
                    "destination_iata": d,
                    "roundtrip": rt,
                    "j": jv,
                    "cpm": cpm,
                    "price": row.get("price"),
                    "airlines": _normalize_text_array(row.get("airlines")),
                    "origin_region": orr,
                    "destination_region": drr,
                }
            if len(base_rows) < page_size:
                break
            off += page_size

        # Merge new rows into pairing candidate pool where cpm < 20.
        for r in records:
            key = _row_key(r)
            o, d, rt, jv = key
            if not o or not d or not rt:
            ...
            if not orr or not drr or cpm is None or cpm >= 20:
                continue
            if (orr, drr) not in pairing_buckets:
                continue
            candidate_rows_by_key[key] = {
                "origin_iata": o,
                "destination_iata": d,
                "roundtrip": rt,
                "j": jv,
        ...
        # Compute flagged keys in pairing pools (bucket by region pair only).
        bucket_items: Dict[tuple[str, str], List[tuple[tuple[str, str, str, str], float]]] = {}
        for key, row in candidate_rows_by_key.items():
            orr = (row.get("origin_region") or "").strip()
            drr = (row.get("destination_region") or "").strip()
            cpm = _to_float(row.get("cpm"))
            if not orr or not drr or cpm is None:
                continue
            bucket_items.setdefault((orr, drr), []).append((key, cpm))

        flagged_keys: set[tuple[str, str, str, str]] = set()
        for bucket, items in bucket_items.items():
            orr, drr = bucket
            if not orr or not drr or orr == drr or drr.lower() == "unknown":
                continue
            if len(items) < 2:
                continue
            items_sorted = sorted(items, key=lambda x: (x[1], x[0]))
            ...
            if gap_curr is not None and gap_next is not None and gap_curr > 0:
                ratio_ok = (gap_next / gap_curr) >= 1.20
            if ratio_ok and breakpoint is not None:
                for key, cpm in items_sorted:
                    if cpm <= breakpoint and cpm < 15.0:
                        flagged_keys.add(key)

        # Apply final pre-upsert flag value to outgoing records.
        new_flag_by_key: Dict[tuple[str, str, str, str], bool] = {}
        outgoing_snapshot_by_key: Dict[tuple[str, str, str, str], Dict[str, Any]] = {}
        for r in records:
            key = _row_key(r)
            o, d, rt, _ = key
            cpm = _to_float(r.get("cpm"))
        ...
        def _is_transient_supabase_502(exc: BaseException) -> bool:
            # Supabase is behind Cloudflare; transient 502 can come back as HTML instead of JSON,
            ...
            return (
                "502" in s
                and (
                    "bad gateway" in s_low
                    or "cloudflare" in s_low
                    or "json could not be generated" in s_low
                    or "json could not be generated" in s
                )
            )

        attempts = max(1, int(SUPABASE_EXTRA_RETRIES))
        delay_seconds = max(0.0, float(SUPABASE_RETRY_DELAY_SECONDS))
        queued_chunks: List[List[Dict[str, Any]]] = []

        def _upsert_chunk(chunk_to_upsert: List[Dict[str, Any]]) -> None:
            res = client.table(tbl).upsert(chunk_to_upsert, on_conflict=on_conflict).execute()
            status_code = 0
            if isinstance(res, dict):
                status_code = int(res.get("status_code", 200) or 200)
            if status_code >= 400:
            ...
            try:
                for att in range(1, attempts + 1):
                    try:
                        ...
                        if _is_transient_supabase_502(exc) and att < attempts:
                            if debug:
                                print(
                                    f"[warn] transient 502 destination upsert queued "
                                    f"(chunk {i // chunk_size + 1}, attempt {att}/{attempts}): {type(exc).__name__}: {exc}",
                                    file=sys.stderr,
                                )
                ...
                if _is_transient_supabase_502(exc):
                    ...
                    if debug:
                        print(
                            f"[warn] destination upsert chunk permanently failed after {attempts} attempts; "
                            f"queued for retry after capture (chunk {i // chunk_size + 1}).",
                            file=sys.stderr,
                        )
                    continue
                raise

        # Retry queued chunk upserts after capture for this origin finishes.
        for idx, chunk in enumerate(queued_chunks, 1):
            for att in range(1, attempts + 1):
                try:
                    _upsert_chunk(chunk)
                    break
                    ...
                    if _is_transient_supabase_502(exc) and att < attempts:
                        if debug:
                            print(
                                f"[warn] retry queued destination upsert (chunk retry {idx}/{len(queued_chunks)}; "
                                f"attempt {att}/{attempts}): {type(exc).__name__}: {exc}",
                                file=sys.stderr,
                            )
                        time.sleep(delay_seconds)
                        continue
        ...
        # Transition diff (pairing-scoped touched keys only).
        became_true_keys: List[tuple[str, str, str, str]] = []
        became_false_keys: List[tuple[str, str, str, str]] = []
        for key, new_val in new_flag_by_key.items():
            old_val = old_flag_by_key.get(key)
            old_is_true = old_val is True
            new_is_true = bool(new_val)
            if (not old_is_true) and new_is_true:
                became_true_keys.append(key)
            elif old_is_true and (not new_is_true):
                became_false_keys.append(key)

        if not became_true_keys and not became_false_keys:
            return

        # Alert transitions + active_route maintenance.
        alert_rows = getattr(
            client.table("google_alerts")
            .select(
                "id,email,type,origin_region,origin_country,origin_airport,"
                "destination_region,destination_country,destination_airport,"
                "airlines_included,airlines_excluded,active_route"
        ...
        airline_name_by_code: Dict[str, str] = {}
        airline_codes_needed: Set[str] = set()
        for key in became_true_keys:
            for c in _normalize_text_array(outgoing_snapshot_by_key.get(key, {}).get("airlines")):
                airline_codes_needed.add(c.upper())
        if airline_codes_needed:
            airline_resp = (
                client.table("airlines")
                .select("code,name")
        ...
        transition_map: Dict[tuple[str, str, str, str], str] = {}
        for k in became_true_keys:
            transition_map[k] = "armed"
        for k in became_false_keys:
            transition_map[k] = "cleared"
        ...
        for alert in alert_rows:
            ...
            active_route_set = set(active_route)
            changed = False
            # Deduped: send at most one summary email per alert row per run.
            email_routes: List[Dict[str, Any]] = []

            for key, transition_kind in transition_map.items():
                snap = outgoing_snapshot_by_key.get(key) or {}
                origin_iata = (snap.get("origin_iata") or "").strip().upper()
                destination_iata = (snap.get("destination_iata") or "").strip().upper()
                roundtrip = (snap.get("roundtrip") or "").strip().lower()
                if not origin_iata or not destination_iata or not roundtrip:
                ...
                if alert_type != "all" and roundtrip != alert_type:
                    continue
                origin_region = (snap.get("origin_region") or "").strip()
                destination_region = (snap.get("destination_region") or "").strip()
                origin_country = (country_by_iata.get(origin_iata) or "").strip().upper()
                destination_country = (country_by_iata.get(destination_iata) or "").strip().upper()
                if not _matches_google_alert_side(
                    side_region_filters=origin_region_filters,
                    side_country_filters=origin_country_filters,
                    side_airport_filters=origin_airport_filters,
            ...
            if changed:
                client.table("google_alerts").update(
                    {"active_route": sorted(active_route_set), "updated_at": datetime.now(timezone.utc).isoformat()}
                ).eq("id", alert_id).execute()
            if email_routes:
                _send_google_explore_alert_summary_email(
                    to_email=to_email,
                    routes=email_routes,
                    debug=debug,
                )
        ...
        if debug:
            print(
                f"[debug] way_too_cheap pre-upsert pairing flow fallback: {type(exc).__name__}: {exc}",
                file=sys.stderr,
            )
        # If pairing-scoped compute fails, fail loudly to avoid silent inconsistent flags.
        raise
...
def upsert_explore_csv_rows_to_supabase(
    ...
    *,
    ...
    upsert_explore_destination_records(
        records,
        output_table=get_supabase_explore_output_table(),
        debug=debug,
    )
